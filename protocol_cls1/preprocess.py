			...
			for name in protocols:
				...
				ip.data.dport == ports[index]:
					if len(flows[index]) >= 10000:
						...
						break
					# match a protocol    '150.5.240.98.124.13.166.178.6.443.50773'生成key
					key = '.'.join(map(str, map(int, ip.src))) + \
					'.' + '.'.join(map(str, map(int, ip.dst))) + \
					'.' + '.'.join(map(str, [ip.p, ip.data.sport, ip.data.dport]))

					if key not in flows[index]:				# flows里面每个元素都是个dict存储kv
						flows[index][key] = [ip]			# key是前面生成的key v是ip数据包    如何判断ip数据包是哪个协议的是根据前面的端口来做的
...
if __name__ == '__main__':
	pcap = dpkt.pcap.Reader(open('/data/zhayaohua/project/trafficls/202006101400.pcap', 'rb'))
