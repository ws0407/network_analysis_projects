...
ROOT_DIR = '/tmp/pycharm_project_704/'
...
def train_epoch(model, training_data, optimizer):
	...
	for batch in tqdm(
		...
		# prepare data
		src_seq, src_seq2, gold = batch			# torch.Size([16384, 50])    torch.Size([16384, 1])
		# src_seq, src_seq2, gold = src_seq.cuda(), src_seq2.cuda(), gold.cuda()

		optimizer.zero_grad()
		# forward
		pred = model(src_seq, src_seq2)
		loss_per_batch, acc_per_batch = cal_loss(pred, gold)		# torch.Size([16384, 6])和torch.Size([16384, 1])
		# update parameters
...
def main(i, flow_dict):
	...
	model_test = SAM(num_class=len(protocols), max_byte_len=max_byte_len)
	# optimizer = optim.Adam(filter(lambda x: x.requires_grad, model.parameters()))
	# loss_list = []

	# train_x, train_y, train_label = load_epoch_data(flow_dict, 'train')
	# training_data = torch.utils.data.DataLoader(
	# 	Dataset(x=train_x, y=train_y, label=train_label),  # 将所有的数据放入Dataset中，get_item时按batch拿
	# 	num_workers=4,
	...
	model_test.load_state_dict(torch.load(ROOT_DIR + "model/{}_fold_checkpoint.pth".format(i), map_location=torch.device('cpu')))

	test_acc, score, pred, test_time = test_epoch(model_test, test_data)

	print("load model test acc == " + str(test_acc))

	header = ['F1', 'PRE', 'REC']
	p, r, fscore, _ = precision_recall_fscore_support(test_label, pred)
	body = []
	for a, b, c in zip(fscore, p, r):
		body.append(['%.2f' % a, '%.2f' % b, '%.2f' % c])
	results.append({
		"type": "table",
		"title": "metric_%d" % i,
		"data": {
	...
	total_test.append(['%.6f' % test_time, '%.2f' % test_acc])
...
if __name__ == '__main__':
	for i in range(10):
		with open(ROOT_DIR + 'data_new/pro_test_flows_%d_noip_fold.pkl'%i, 'rb') as f:
			flow_dict = pickle.load(f)
		# del flow_dict['train']
		#
		# with open('pro_test_flows_%d_noip_fold.pkl'%i, 'wb') as f2:
		# 	pickle.dump(flow_dict, f2)
