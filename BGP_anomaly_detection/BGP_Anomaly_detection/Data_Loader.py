...
class Data_Loader(object):
    ...
    def __load_pd_dataset(self,include_MANRS_data=True,baseline=False):
        datasets_path = ROOT_DIR + 'datasets/datasets/'
        # load files path
        ...
        if include_MANRS_data:
            datasets_path2 = ROOT_DIR + 'datasets/legitimate/'
    ...
    def __loadDataCol(self,read_from_file=False):
        if read_from_file:
            data_all = pd.read_csv(ROOT_DIR + 'result_doc/data_all.csv', index_col=0)
            ...
            # change test features to train features
            data_all.to_csv(ROOT_DIR + 'result_doc/data_all.csv')
            self.data_cols = data_all.columns
