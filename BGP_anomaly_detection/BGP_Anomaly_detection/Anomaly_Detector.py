...
ROOT_DIR = '/tmp/pycharm_project_BGP/'
results = []
...
class Detector(object):
    ...
    def train(self,Event_num,read_from_file=True, include_MANRS_data=True,WINDOW_SIZE=30,HIDDEN_SIZE=128,save_epoch=10): #the implement of training model
        ...
        Path = ROOT_DIR + 'params/best_lstm_params_' + Event_name + '.pkl'
        ...
        for epoch in range(self.EPOCH):
            for step, (x, y) in tqdm(enumerate(train_loader)):
                ...
                if step % 10000 == 0:
                    ...
                    if(epoch==save_epoch):
                        print(temp_str + '\n' + str(temp_f1))
                        with open(ROOT_DIR + 'result_doc/test_best_f1' + Event_name + '.txt', 'a') as f:
        ...
        lstm.load_state_dict(torch.load(Path, map_location=torch.device('cpu')))
        if SA_LSTM_flag:
        ...
        test_report = classification_report(y_true=test_y, y_pred=pred_y,
                                         target_names=target_list)
        test_parameter_path = ROOT_DIR + 'result_doc/test_parameter' + '_' + Event_name + '.txt'
        ...
        #attn_weights_df.to_csv(ROOT_DIR + 'result_doc/atten_weights' + '_' + Event_name + '.csv')
        torch.save(lstm, ROOT_DIR + 'params/lstm' + Event_name + '.pkl')
    ...
    def test_route_leak(self):
        ...
        for Event_name in Event_list:
            scaler = pickle.load(open(ROOT_DIR + 'params/' + Event_name + '_scaler.pkl', 'rb'))
            ...
            from sklearn.metrics import classification_report
            import pandas
            x_ = classification_report(y_true=test_y, y_pred=pred_y, target_names=['Normal', 'Abnomal '])
            print(x_)
        ...
        true_pred.to_csv(ROOT_DIR + 'result_doc/pred_true_route_leak_train.csv')
        ...
        for epoch in range(self.EPOCH):
            for step, (x, y) in tqdm(enumerate(train_loader)):
                ...
                if step % 1400 == 0:
                    ...
                    if (epoch==save_epoch):
                        print(temp_str + '\n' + str(temp_f1))
                        with open(ROOT_DIR + 'result_doc/retrain' + Event_name + '.txt', 'a') as f:
                            message = 'epoch:' + str(epoch) + ' f1_score:' + str(temp_f1) + '\n'
                            f.write(message)
                        best_f1_score = temp_f1
                        torch.save(model.state_dict(), ROOT_DIR + 'params/retrain' + Event_name + Scheme+'.pkl')

        path = ROOT_DIR + 'params/retrain' + Event_name + Scheme+'.pkl'
        model.load_state_dict(torch.load(path, map_location=torch.device('cpu')))
    ...
    def plot_test_route_leak(self):
        ...
        import pandas as pd
        pred = pd.read_csv(ROOT_DIR + "result_doc/pred_true_route_leak_train.csv")
        ...
        plt.xticks(x, tick)
        plt.legend()
        plt.savefig(ROOT_DIR + 'result/case_study.png', bbox_inches='tight')
        # results.append({
        #     "type": "image",
    ...
    def transfer_learning(self,vote_thr=2,Scheme='A'): #transfer to the well-known anomalies
        ...
        self.true_pred.to_csv(ROOT_DIR + 'result_doc/retrain_'+Scheme+'.csv')
    ...
    def test(self,data_path, event_name, window): #API for testing on the other datasets
        ...
        for Event_name in Event_list:
            ...
            true_pred['pred_' + Event_name] = pred_y
        true_pred['true'] = test_y
        true_pred.to_csv(ROOT_DIR + 'result_doc/pred_true_' + event_name + '.csv')
    ...
    def baseline(self, Event_num, read_from_file=True, include_MANRS_data=True,baseline_Feature=False): #baseline method SVM,RF
        ...
        for score in scores:
            ...
            for clf in models:
                ...
                baseline_txt_path = ROOT_DIR + 'result_doc/baseline.txt'
...
detector.plot_test_route_leak()

results.append({
    "type": "text",
    "title": "2. 分析正常与异常路由行为特征，并以此特征为判断异常的输入根据",
    "data": ""
})

results.append({
    "type": "image",
    "title": "(1) 正常路由前缀行为规范（上图），与异常路由前缀行为（下图）",
    "description": "Null",
    "data": "box1.png"
})
...
{
    "type": "image",
    "title": "(2) 正常路由宣告行为规范（上图），与异常路由宣告行为（下图）",
    "description": "Null",
    "data": "box2.png"
},
{
    "type": "text",
    "title": "3. 正常路由AS-PATH行为规范与异常路由宣告行为分析",
    "data": ""
},
{
    "type": "image",
    "title": "(1) Heavy tail distribution 1",
    "description": "90th quantile distribution plot of path length.",
    "data": "len.png"
},
{
    "type": "image",
    "title": "(2) Heavy tail distribution 2",
    "description": "90th quantile distribution plot of edit distance.",
    "data": "Diff.png"
},
{
    "type": "image",
    "title": "(3) Heavy tail distribution 3",
    "description": "90th quantile distribution plot of increase path length.",
    "data": "Longer.png"
},
{
    "type": "image",
    "title": "(4) Heavy tail distribution 4",
    "description": "90th quantile distribution plot of decrease path length.",
    "data": "Shorter.png"
},
{
    "type": "text",
    "title": "4. 查看公开测试集上的预测结果",
    "data": ""
},
{
    "type": "image",
    "title": "Anomaly detection result",
    "description": "公开测试集上的异常检测结果。",
    "data": "test_route_leak.png"
}]
